...
layui.define(['table', 'form'], function (exports) {
  ...
  table.render({
    ...
    url: 'http://localhost:8007/role', //模拟接口
    headers,
    cols: [
      [{
          type: 'checkbox',
  ...
  table.on('tool(LAY-app-content-list)', function (obj) {
    ...
    if (obj.event === 'del') {
      layer.confirm('确定删除此角色？', function (index) {
        ...
        admin.req({
          type: 'DELETE',
          url: 'http://localhost:8007/role/' + data.id,
      ...
      admin.popup({
        ...
        success: function (layero, index) {
          view(this.id).render('app/content/roleform', data).done(function () {
            ...
            form.on('submit(layuiadmin-app-form-submit)', function (data) {
              ...
              admin.req({
                type: 'PUT',
                url: 'http://localhost:8007/role/' + id,
                data: data,
                success: function (res) {
